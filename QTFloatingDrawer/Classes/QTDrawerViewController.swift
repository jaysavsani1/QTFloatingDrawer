...
public enum QTDrawerSide: CGFloat {
    case none  = 0
    case left  = 1
    case right = -1
}
...
open class QTDrawerViewController: UIViewController{
    ...
    fileprivate var _drawerView: QTDrawerView?
    var drawerView: QTDrawerView {
        get {
            if let retVal = _drawerView {
                return retVal
            }
            let rect = UIScreen.main.bounds
            let retVal = QTDrawerView(frame: rect)
            _drawerView = retVal
            return retVal
        }
    }
    ...
    fileprivate var _animator: QTDrawerSpringAnimator?
    open var animator: QTDrawerSpringAnimator {
        get {
            if let retVal = _animator {
                return retVal
            }
            let retVal = QTDrawerSpringAnimator()
            _animator = retVal
            return retVal
        }
    }
    
    open func openDrawer(_ side: QTDrawerSide, animated:Bool, complete: @escaping (Bool) -> Void) {
    ...
    open func closeDrawer(_ side: QTDrawerSide, animated: Bool, complete: @escaping (Bool) -> Void) {
    ...
    open func toggleDrawer(_ side: QTDrawerSide, animated: Bool, complete: @escaping (_ finished: Bool) -> Void) {
        if side != .none {
    ...
    func viewContainer(_ side: QTDrawerSide) -> UIViewController? {
        switch side {
        case .left:
    ...
    open var currentlyOpenedSide: QTDrawerSide = .none
    ...
    fileprivate lazy var toggleDrawerTapGestureRecognizer: UITapGestureRecognizer = {
        [unowned self] in
        let gesture = UITapGestureRecognizer(target: self, action: #selector(QTDrawerViewController.centerViewContainerTapped(_:)))
        return gesture
        }()
